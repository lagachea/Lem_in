...
/*   Updated: 2020/05/09 13:01:10 by plagache         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static	int	fail_links(t_list *head)
{
	t_list	*ptr;
	t_room	*room;
...
static	int	duplicate_room(t_list *head)
{
	t_list	*ptr;
	char	*str1;
	char	*str2;
...
static	int	command(t_list *start, t_list *end)
{
	if (start == end)
		return (FAILURE);
	return (SUCCESS);
...
static	int	ultimate_path(t_list *start, t_list *end)
{
	t_list	*ptr;
